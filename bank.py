...
def bank_moneyOut(password,moneyOut_num, username):
    if username in users:
        if password==users[username]["password"]:
            if moneyOut_num<=users[username]["money"]:
                return 0
            else:
                return 3
        else:
...
def moneyOut():
    ...
    moneyOut_num=int(input("请输入您要取出的金额"))

    status2 = bank_moneyOut(password,moneyOut_num, username)
    if status2==0:
        print("取款",moneyOut_num,"元成功，","您的余额为",users[username]["money"]-moneyOut_num)
    elif status2==1:
        print("对不起，您的账户不存在，请重新输入")
        return moneyOut()
    elif status2==2:
        print("对不起，您输入的密码不正确，请重新输入")
        return moneyOut()
    elif status2==3:
        print("对不起，您的余额不足")
